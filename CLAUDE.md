...
| Domain | `reilusoppari.fi`, www → apex. Sovellus myöhemmin `app.reilusoppari.fi` (eSinetin moottori, oma brändi). |
...
1. **Hero.** Otsikko: "Vuokrasopimus, jonka molemmat allekirjoittavat mielellään." Alaotsikko: "Reilusoppari hoitaa sopimuksen, kuvat asunnon kunnosta, vuokrakuittaukset ja lopuksi todistuksen – molemmille. Ensimmäinen vuokrasuhde on ilmainen." CTA `<PrimaryCta />` ("Aloita ilmaiseksi" / odotuslista), alle "Vuokralaiselle aina maksuton."
   Heron visuaalinen elementti: **kaksipuolinen näkymä**. Vasemmalla vuokranantajan puhelin, oikealla vuokralaisen puhelin, keskellä sama sopimus. Vaihtokytkin heron yläreunassa "Olen vuokranantaja / Olen vuokralainen" vaihtaa alaotsikon ja CTA-tekstin (vuokralaiselle: "Pyydä vuokranantajaa käyttämään Reilusopparia" → valmis viestipohja jaettavaksi). Kytkimen tila muistetaan sivuston sisällä (URL-parametri, ei evästettä).
...
5. **Vuokrakuittaus.** Teksti sitova (Jukan kirjoittama):
   ...
   Vieressä kuva ilmoituksesta: "Maksoiko Liisa 850 € eräpäivään 2.9. mennessä?" ja napit Kyllä · Ei vielä · Osittain. Alle yksi asiallinen lause: "Vuokralainen näkee kuittauksen heti, ja vuoden päästä kummallakin on sama maksuhistoria – ilman pankkitiliä, ilman perintätoimistoa." Huom: tämä on sivun toinen "Reilua."-loppu; ohjeen sääntö yhdestä per sivu väistyy, kun tekstit ovat Jukan – mutta kolmatta ei lisätä.
